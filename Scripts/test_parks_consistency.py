...
class TestConsistency(unittest.TestCase):
    ...
    def test_park_code_consistency(self):
        ...
        # Check for consistency between records.csv and parks_points.geojson
        if records_codes != points_codes:
            ...
            if missing_in_points:
                print(f"Missing in parks_points.geojson but in records.csv: {missing_in_points}")
            if extra_in_points:
                print(f"Extra in parks_points.geojson not in records.csv: {extra_in_points}")
            self.assertEqual(records_codes, points_codes, "Mismatch between records.csv and parks_points.geojson")

        # Check for consistency between records.csv and parks_shapes.geojson
        if records_codes != shapes_codes:
            ...
            if missing_in_shapes:
                print(f"Missing in parks_shapes.geojson but in records.csv: {missing_in_shapes}")
            if extra_in_shapes:
                print(f"Extra in parks_shapes.geojson not in records.csv: {extra_in_shapes}")
